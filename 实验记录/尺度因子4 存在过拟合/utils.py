...
def augment(img,hflip=True,rot=True,scale=True):
    ...
    # s=random.choice([0.9, 0.8, 0.7,1])
    # s=random.randint(5,10)*0.1
    ...
    # img = img.resize((int(img.width * s), int(img.height * s)), resample=Image.BICUBIC)
    return img
...
def preprocess_to_y(img, device='cpu'):
    ...
    ycbcr = np.array(img).astype(np.float32)   ##将PIL图片转换为numpy  数据类型是np.float32
    x = ycbcr[..., 0]
    x /= 255.
    x = torch.from_numpy(x).to(device)
    x = x.unsqueeze(0).unsqueeze(0)
    return x, img  ####这里是为了产生测试图片  所以会转换为tensor格式
...
def ssim(img1,img2):
    ...
    if img1.shape[2]==1:
        img1=img1.squeeze(2)
        img2=img2.squeeze(2)
        out=compare_ssim(img1,img2,multichannel=False)
    else:
...
def save_checkpoint(path,model, epoch,optimizer,lr,best_epoch,best_psnr):
